...
const SelectItem = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<typeof SelectPrimitive.Item> & { 
    value: string  // Explicitly add value as required prop to enforce non-empty value
  }
>(({ className, children, ...props }, ref) => {
  // Safety check to ensure value is never an empty string
  const value = props.value?.trim() ? props.value : `default-${Math.random().toString(36).substring(2, 9)}`;
